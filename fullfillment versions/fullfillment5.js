...
exports.dialogflowFirebaseFulfillment = functions.https.onRequest((request, response) => {
  ...
  async function test(agent) {
    try {
      const location = agent.getContext("facebook_location").parameters;
      var latString = location.lat.toString();
      var longString = location.long.toString();
      var url = `https://gopray.com.au/?gmw_post=place&gmw_address%5B%5D=Locations+Near+You&gmw_distance=50&gmw_units=metric&gmw_form=1&paged=1&gmw_per_page=3&gmw_lat=${latString}&gmw_lng=${longString}&gmw_px=pt&action=gmw_post`;
      agent.add(`Prayer locations and Iqamah times can be found at: \n${url}`);
    }
    catch (err) {
    ...
    .then(scrapeMosques)
    .catch(function(err){
      console.log(`error whilst scraping mosques: ` + err);
    })
    .finally(function () {
  ...
  function scrapeMosques(html){
    ...
    if (soup.find('ul', 'posts-list-wrapper')) { //if theres a list of mosques on gopray
      ...
      mosqueurls.forEach(mosqueurl => { // scrape each url
        rp(mosqueurl)
	      .then(scrapeMosqueTimes)
	      .catch(function(err){
          console.log(`error whilst scraping mosque times: ` + err);
        });
      });
    }
    else console.log("Failed soup, probably no mosques");
    return;
  }
